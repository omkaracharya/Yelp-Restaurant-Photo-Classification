...
test_photo_paths = [os.path.join(DATA_HOME + 'test_photos/', str(photo_id) + '.jpg') for photo_id in
                    test_data['photo_id']]

# Each batch will have 500 images for feature extraction (Each batch contributes to 0.04% of the test data approx)
test_size = len(test_photo_paths)
batch_size = 500
batch_number = already_extracted_images / batch_size + 1

print "Total images:", test_size
print "Already Done Images: ", already_extracted_images
...
for image_count in range(already_extracted_images, test_size, batch_size):
    ...
    print "Batch No:", batch_number, "\tStart:", image_count, "\tEnd:", image_count + batch_size, "\tTime required:", time.time() - start_time, "sec", "\tCompleted:", float(
        image_count + batch_size) / float(test_size) * 100, "%"
    batch_number += 1
